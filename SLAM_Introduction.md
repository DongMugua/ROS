...
### 1.1 函数、参数的声明及初始化
main函数开始之前，声明函数、初始化变量；定义常量；定义点云类型。
```
    // 相机图像接收频率
    #define FPS 30
    ...
    float m_max_z_ = 8.0;
```
### 1.2 相机管道配置以及深度图像向色彩图像的对齐
在realsense SDK2.0中，是通过管道获取相机的色彩帧和深度帧。所以初始化时，我们配置了两个数据流——16位单通道的深度数据流和8位三通道的RGB色彩数据流，以及30Hz的接收频率。
```
    // 创建一个管道以及管道的参数变量
    rs2::pipeline pipe;
    rs2::config p_config;
...
```
在此，我们需要申明一个能够实现深度图向其他图像对齐的align类型的变量**align**，在后续的代码中，我们将通过此变量实现深度帧的对齐。
```
