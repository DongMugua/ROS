...
/**
 * A Manifold construct to handle state spaces with singularities and
 * encapsualte
...
struct Manifold {
  ...
  using Vectord =
      Eigen::Matrix<double, DOF, 1>;  // Handle the element like a vector
...
class UnscentedKalmanFilter {
 public:
  using CovarianceType =
      Eigen::Matrix<double, DOF, DOF>;  // The covariance size to use
  template <unsigned N>
 ...
 private:
  template <typename T>
  using SigmaArray =
      std::array<T, DOF * 2 + 1>;  // The array type for the sigma points

 public:
  State mean;  // The mean of the hypothesis that is generated
  CovarianceType cov = CovarianceType::Zero();  // The covariance of the
                                                // hypothesis to quantify the
                                                // certainty
 ...
 public:
  /**
   * The constructor for the filter that requires an initial state to start
   * with.
  ...
  /**
   * Initalization function to start the process. Setting the mean, covariance
   * and sigma points.
   * @param initState, the mean to be set
   * @param initNoise, the noise as a standard deviation to initialize the
   * covariance (the given matrix will be squared)
   */
  void init(const State& initState, const CovarianceType& initNoise);

  /**
   * The prediction step to propagate the whole hypothesis with a given dynamic
   * model and an operation specific noise.
   ...
   */
  void predict(std::function<void(State&)> dynamicModel,
               const CovarianceType& noise);

  /**
   * The multi dimensional update step to integrate a measurement into an
   * existing hypothesis.
   ...
   * @param measurementNoise, the measurement specific noise to quantify the
   * uncertainty
   */
  template <unsigned N>
  void update(const Vectord<N>& measurement,
              std::function<Vectord<N>(const State&)> measurementModel,
              const Eigen::Matrix<double, N, N>& measurementNoise);

  /**
   * The single dimensional update step to integrate a measurement into an
   * existing hypothesis.
   * In other works this function is referred as measurement step.
  ...
  void update(double measurement,
              std::function<double(const State&)> measurementModel,
              double measurementNoise);

 private:
  /**
   * The helper function to calculate sigma points using the given mean and the
   * covariance.
   */
 ...
 */
template <typename State, unsigned DOF, bool Manifold>
UnscentedKalmanFilter<State, DOF, Manifold>::UnscentedKalmanFilter(
...
/**
 * Initalization function to start the process. Setting the mean, covariance and
 * sigma points.
 * @param initState, the mean to be set
 * @param initNoise, the noise as a standard deviation to initialize the
 * covariance (the given matrix will be squared)
 */
template <typename State, unsigned DOF, bool Manifold>
void UnscentedKalmanFilter<State, DOF, Manifold>::init(
    const State& initState, const CovarianceType& initNoise) {
  mean = initState;
...
void UnscentedKalmanFilter<State, DOF, Manifold>::predict(
    std::function<void(State&)> dynamicModel, const CovarianceType& noise) {
  if (!((noise.array() >= 0.f).all())) return;

  if (!(noise.trace() > 0.f)) return;
...
/**
 * The multi dimensional update step to integrate a measurement into an existing
 * hypothesis.
 * In other works this function is referred as measurement step.
 * @param measurement, a vector that stores all relevant data of a measurement
 * @param measurementModel, a function that returns a measurement for a state
...
void UnscentedKalmanFilter<State, DOF, IsManifold>::update(
    const Vectord<N>& measurement,
    std::function<Vectord<N>(const State&)> measurementModel,
    const Eigen::Matrix<double, N, N>& measurementNoise) {
  if (!((measurementNoise.array() >= 0.).all())) return;
  if (!(measurementNoise.trace() > 0.)) return;
...
template <typename State, unsigned DOF, bool IsManifold>
void UnscentedKalmanFilter<State, DOF, IsManifold>::update(
    double measurement, std::function<double(const State&)> measurementModel,
    double measurementNoise) {
  if (!(measurementNoise > 0.)) return;
  ...
  double z = 0.f;
  for (double& Zi : Z) z += Zi;
  z /= static_cast<double>(sigmaPoints.size());

  double sigmaz = 0.;
  for (double& Zi : Z) sigmaz += sqr(Zi - z);
  sigmaz *= 0.5;
 ...
 */
template <typename State, unsigned DOF, bool IsManifold>
inline State UnscentedKalmanFilter<State, DOF, IsManifold>::meanOfSigmaPoints()
    const {
  return MeanOfSigmaPoints<State, DOF, SigmaArray<State>, IsManifold>::run(
      sigmaPoints);
}
...
template <typename State, unsigned DOF, bool IsManifold>
inline void
UnscentedKalmanFilter<State, DOF, IsManifold>::covarianceMatrixValidation(
    const CovarianceType& cov) const {
  for (unsigned i = 0; i < DOF; ++i) {
    // ASSERT(cov(i, i) > 0.f);
...
struct MeanOfSigmaPoints<State, DOF, Array, true> {
  using Vectord =
      Eigen::Matrix<double, DOF, 1>;  // The vector type to operate on
  ...
  static State run(const Array& sigmaPoints) {
    ...
    // Move the mean each step into the direction of the sigma points till the
    // difference is minimal or the limit is reached
    do {
      lastMean = mean;
      ++iterations;
...
/**
 * A struct to run a calculation of mean for given sigma points without
 * manifold.
 */
template <typename State, unsigned DOF, typename Array>
...
using UKF =
    impl::UnscentedKalmanFilter<Eigen::Matrix<double, DOF, 1>, DOF, false>;
