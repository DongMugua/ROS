...
#if defined(__linux__)
uint32_t GroupBulkRead::getData(uint8_t id, uint16_t address, uint16_t data_length, int32_t *dataArray)
{
  if (isAvailable(id, address, data_length) == false)
    return 0;
  ...
  {
    case 1:
      dataArray[0]=data_list_[id][address - start_addr];
      return data_list_[id][address - start_addr];

    case 2:
      dataArray[0]=data_list_[id][address - start_addr];
      dataArray[1]=data_list_[id][address - start_addr + 1];
      return DXL_MAKEWORD(data_list_[id][address - start_addr], data_list_[id][address - start_addr + 1]);

    case 4:
      dataArray[0]=data_list_[id][address - start_addr + 0];
      dataArray[1]=data_list_[id][address - start_addr + 1];
      dataArray[2]=data_list_[id][address - start_addr + 2];
      dataArray[3]=data_list_[id][address - start_addr + 3];
      return DXL_MAKEDWORD(DXL_MAKEWORD(data_list_[id][address - start_addr + 0], data_list_[id][address - start_addr + 1]),
                           DXL_MAKEWORD(data_list_[id][address - start_addr + 2], data_list_[id][address - start_addr + 3]));
    default:
      for (uint8_t i=0; i<data_length; i++)
      dataArray[i] = data_list_[id][address - start_addr + i];
      return 0;
  }
}
#endif
